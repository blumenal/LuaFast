...
async def get_latest_repo_info(CLIENT, repos: list, app_id: str, headers) -> Tuple[Optional[str], Optional[str]]:
    async def fetch_repo_data(repo: str) -> Tuple[Optional[dict], str]:
        ...
        try:
            ...
            LOG.warning(f"Falha ao buscar repositório {repo}: {str(e)}")
            return None, repo
    ...
    for repo_data, repo_name in results:
        if repo_data and "commit" in repo_data:
            commit_date = repo_data["commit"]["commit"]["author"]["date"]
...
async def handle_depot_files(CLIENT, repos: List, app_id: str, steam_path: Path) -> Tuple[List[Tuple[str, str]], Dict]:
    collected = []
    depot_map = {}
    try:
...
if __name__ == "__main__":
    async def async_main():
        try:
            ...
            LOG.error(f"Erro fatal: {stack_error(e)}")
